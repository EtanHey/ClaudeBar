...
struct {Component}Tests {
    ...
    @Test func `{describes correct behavior}`() {
        ...
        let Codex = ClaudeProvider(probe: MockUsageProbe(), settingsRepository: settings)
        Codex.isEnabled = false  // Bug trigger condition

        // When - action that should work correctly
        let monitor = QuotaMonitor(providers: AIProviders(providers: [Codex, codex]))

        // Then - assert EXPECTED behavior (will FAIL before fix)
        #expect(monitor.selectedProviderId == "codex")  // Not "Codex"
    }
}
```
